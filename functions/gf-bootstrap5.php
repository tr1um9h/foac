...
if (class_exists('GFCommon')) {
  ...
  add_filter('gform_field_content', function ($content, $field, $value, $lead_id, $form_id) {
    ...
    // Validation message
    $content = str_replace('class=\'small text-muted gfield_description validation_message', 'class=\'text-danger p-1 small gfield_description validation_message', $content);
    ...
    // Select fields.
    $content = str_replace('gfield_select', 'form-select empty', $content);
    ...
    // Date fields.
    if ('date' === $field['type']) {
      $content = str_replace('<select', '<select class=\'form-select empty\'', $content);
      $content = str_replace('ginput_complex', 'row ginput_complex', $content);
    ...
    if ('time' === $field['type']) {
      $content = str_replace('<select', '<select class=\'form-select empty\'', $content);
      $content = str_replace('ginput_complex', 'row g ginput_complex', $content);
    ...
    if ('name' === $field['type'] || 'address' === $field['type'] || 'email' === $field['type'] || 'password' === $field['type']) {
      $content = str_replace('class=\'ginput_complex', 'class=\'row ginput_complex', $content);
    ...
    if ('name' === $field['type'] || 'address' === $field['type']) {
      $content = str_replace('class=\'name_', 'class=\'col-12 col-md-6 name_', $content);
      $content = str_replace('type=\'text\'', 'type=\'text\' class=\'form-control\'', $content);
      $content = str_replace('<select ', '<select class=\'form-select empty\' ', $content);
      $content = str_replace('label for=', 'label class=\'small text-muted visually-hidden\' for=', $content);
    }
  ...
  add_filter('gform_submit_button', function ($button, $form) {
    $button = str_replace('class=\'gform_button', 'class=\'gform_button btn btn-interactive', $button);
    return $button;
  }, 10, 2);
